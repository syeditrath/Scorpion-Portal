...
const CAT = {
  TUV:       { color: T.blue,  dim: T.blueDim,  icon: "◈", label: "TUV Certifications" },
  Manpower:  { color: T.green, dim: T.greenDim, icon: "◉", label: "Manpower Certifications" },
  Equipment: { color: T.gold,  dim: T.goldDim,  icon: "◎", label: "Equipment Certifications" },
};
...
const EXCEL_MAP = {
  TUV: {
    ...
    "EQUIPMENT ID /SERIAL NUMBER":"serialId",
...
function parseExcelForCategory(data, category) {
  ...
  return data
    ...
    .map(row => {
      ...
      Object.entries(map).forEach(([excelCol, appKey]) => {
        ...
        if (val !== undefined && val !== null && val !== "") {
          if (appKey === "expiryDate" || appKey === "inspectionDate" || appKey === "issueDate") {
            rec[appKey] = excelDateToString(val);
          } else {
            rec[appKey] = String(val).trim();
          }
        }
      });
      return rec;
    ...
    .filter(r => {
      const primary = primaryOf(category, r);
      return primary && primary !== "—";
    });
}

function loadRecords() {
  try { const r = localStorage.getItem("ct_v4"); return r ? JSON.parse(r) : SEED; }
  catch { return SEED; }
}
function persist(data) {
  try { localStorage.setItem("ct_v4", JSON.stringify(data)); } catch {}
}
...
export default function App() {
  useEffect(() => {
    if (!document.getElementById("ct-g")) {
      ...
      s.id = "ct-g"; s.textContent = GLOBAL_CSS;
  ...
  const saveRec = (category, data, mode) => {
    ...
    showToast(mode==="add" ? "Record added successfully" : "Record updated");
  ...
  const importExcel = (category, file) => {
    ...
    reader.onload = e => {
      try {
        ...
        if (parsed.length === 0) { showToast("No valid rows found in file", "del"); return; }
        setRecs(prev => ({...prev, [category]: parsed}));
        showToast(`✓ Imported ${parsed.length} records into ${category}`, "ok");
      } catch(err) {
  ...
  return (
    <div style={{display:"flex", height:"100vh", overflow:"hidden", background:T.bg}}>
      ...
      {/* mobile overlay */}
      {sideOpen && (
        <div className="fade-in" onClick={() => setSideOpen(false)}
          style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",zIndex:49}} />
      )}

      {/* ══ SIDEBAR ══ */}
      <Sidebar tab={tab} go={go} stats={stats} attention={attention} sideOpen={sideOpen} setSideOpen={setSideOpen} />
      ...
      <div style={{flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0}}>
        ...
        <main style={{flex:1, overflowY:"auto", padding:"24px 20px"}}>
          ...
          {tab==="tracker"   && <Tracker cat={cat} setCat={setCat} filtered={filtered} stats={stats} onAdd={()=>setModal({mode:"add",category:cat})} onEdit={r=>setModal({mode:"edit",category:cat,record:r})} onDel={id=>delRec(cat,id)} onDetail={r=>setDetail({...r,cat})} onImport={importExcel} />}
      ...
      {toast && (
        <div className="fade-up" style={{
          ...
          background: toast.type==="del" ? "#130a0a" : "#081310",
          border:`1px solid ${toast.type==="del" ? T.red : T.green}`,
          color: toast.type==="del" ? T.red : T.green,
...
/* ════════════════════════════════════════════════════════════════════════════
   SIDEBAR
════════════════════════════════════════════════════════════════════════════ */
function Sidebar({ tab, go, stats, attention, sideOpen, setSideOpen }) {
      ...
      <div style={{padding:"20px 18px 16px", borderBottom:`1px solid ${T.border}`}}>
        <div style={{display:"flex", alignItems:"center", gap:12}}>
          <img src="logo.png" alt="Scorpion Arabia"
            style={{width:40, height:40, borderRadius:8, objectFit:"cover", background:"#000"}} />
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:16, color:T.text, letterSpacing:".3px", lineHeight:1.1}}>SCORPION ARABIA</div>
            <div style={{fontSize:10, color:T.textMuted, fontWeight:600, letterSpacing:"1.2px", marginTop:2}}>CERT TRACKER</div>
          </div>
        </div>
      </div>
      ...
      <nav style={{padding:"14px 10px", flex:1}}>
        ...
        {[
          ...
          <button key={n.id} onClick={() => go(n.id)} style={{
            ...
            textAlign:"left",
            background: tab===n.id ? T.blueDim : "transparent",
            borderLeft: `2px solid ${tab===n.id ? T.blue : "transparent"}`,
            transition:"all .15s",
          }}>
            ...
            {n.badge > 0 && (
              <span style={{background:T.red, color:"#fff", borderRadius:999, padding:"1px 7px", fontSize:10, fontWeight:700, flexShrink:0}}>{n.badge}</span>
            )}
          </button>
        ))}
        ...
        {CATS.map(c => {
          const cfg = CAT[c];
          return (
            <button key={c} onClick={() => { go("tracker"); }} style={{
              width:"100%", display:"flex", alignItems:"center", gap:10,
              padding:"8px 12px", borderRadius:8, border:"none", marginBottom:2,
              background:"transparent", textAlign:"left", transition:"background .15s",
            }}
...
/* ════════════════════════════════════════════════════════════════════════════
   TOP BAR
════════════════════════════════════════════════════════════════════════════ */
function TopBar({ tab, cat, allProjects, search, setSearch, fProj, setFProj, fStat, setFStat, attention, onAdd, onAlertCfg, onHamburger, onImport, currentCat }) {
  const fileRef = useRef();
  const titles  = { dashboard:"Dashboard", tracker:`${cat} Certifications`, alerts:"Alerts" };
  ...
  return (
    <header style={{background:T.sidebar, borderBottom:`1px solid ${T.border}`, padding:"0 20px", flexShrink:0}}>
      <div style={{display:"flex", alignItems:"center", gap:12, height:56}}>
        {/* Hamburger — always visible, functional on all sizes */}
        <button onClick={onHamburger}
          style={{background:T.card, border:`1px solid ${T.border}`, color:T.textSub, borderRadius:8, width:36, height:36, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0}}>
          ☰
        </button>

        <div style={{flex:1, fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:18, color:T.text, letterSpacing:".5px"}}>
          {titles[tab]}
        </div>

        <div style={{display:"flex", gap:8, alignItems:"center"}}>
          {/* Excel import button — tracker only */}
          {tab==="tracker" && (
            <>
              <input ref={fileRef} type="file" accept=".xlsx,.xls" style={{display:"none"}}
                onChange={e => { if(e.target.files[0]) { onImport(currentCat, e.target.files[0]); e.target.value=""; }}} />
              <button onClick={() => fileRef.current.click()} style={{
                background:T.goldDim, border:`1px solid ${T.gold}44`,
                color:T.gold, borderRadius:8, padding:"7px 14px",
                fontSize:12, fontWeight:600, display:"flex", alignItems:"center", gap:6,
              }}>
                ⬆ Import Excel
              </button>
              <button onClick={onAdd} style={{
                background:CAT[currentCat].color, color:"#000",
          ...
          {/* Alert bell */}
          {attention.length > 0 && (
            <button onClick={onAlertCfg} style={{
              background:T.redDim, border:`1px solid ${T.red}44`,
              color:T.red, borderRadius:8, padding:"7px 12px",
              fontSize:12, fontWeight:600, display:"flex", alignItems:"center", gap:6,
            }}>
      ...
      {tab==="tracker" && (
        <div style={{paddingBottom:12, display:"flex", gap:8, flexWrap:"wrap"}}>
          ...
          <select value={fProj} onChange={e=>setFProj(e.target.value)}
            style={{background:T.inputBg, border:`1px solid ${T.border}`, borderRadius:8, padding:"8px 12px", fontSize:13, color:T.textSub, outline:"none", colorScheme:"dark"}}>
  ...
  return (
    <div style={{maxWidth:1100, margin:"0 auto"}}>
      ...
      <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:14, marginBottom:22}}>
        {[
          {label:"Total Certifications", val:total,    color:T.blue,  icon:"▦"},
          {label:"Valid",                val:valid,    color:T.green, icon:"✓"},
          {label:"Expiring in 90 Days",  val:expiring, color:T.gold,  icon:"▲"},
          {label:"Expired",              val:expired,  color:T.red,   icon:"✕"},
        ].map((m,i) => (
          <div key={m.label} className="fade-up"
            style={{background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:"20px 22px", animationDelay:`${i*.06}s`, position:"relative", overflow:"hidden"}}>
      ...
      <div className="fade-up" style={{background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:"18px 22px", marginBottom:20, animationDelay:".26s"}}>
        ...
        <div style={{height:8, background:T.border, borderRadius:999}}>
          <div style={{height:"100%", width:`${pct}%`, borderRadius:999, transition:"width .8s ease",
            background:pct>=80?`linear-gradient(90deg,${T.green},#059669)`:pct>=60?`linear-gradient(90deg,${T.gold},#d97706)`:`linear-gradient(90deg,${T.red},#dc2626)`}} />
        </div>
...
/* ════════════════════════════════════════════════════════════════════════════
   TRACKER
════════════════════════════════════════════════════════════════════════════ */
function Tracker({ cat, setCat, filtered, stats, onAdd, onEdit, onDel, onDetail, onImport }) {
  const fileRef = useRef();
  return (
    <div style={{maxWidth:1100, margin:"0 auto"}}>
      ...
      <div style={{background:T.goldDim, border:`1px solid ${T.gold}33`, borderRadius:12, padding:"12px 16px", marginBottom:16, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10}}>
        <div>
          ...
          <div style={{fontSize:11, color:T.textMuted, marginTop:2}}>Upload your {cat} Excel file — all existing records will be replaced</div>
        </div>
        <div>
          <input ref={fileRef} type="file" accept=".xlsx,.xls" style={{display:"none"}}
            onChange={e => { if(e.target.files[0]) { onImport(cat, e.target.files[0]); e.target.value=""; }}} />
          <button onClick={() => fileRef.current.click()} style={{
            background:T.gold, color:"#000", border:"none", borderRadius:8,
            padding:"8px 18px", fontSize:13, fontWeight:700, display:"flex", alignItems:"center", gap:6,
          }}>⬆ Upload {cat} Excel</button>
        </div>
      </div>

      <div style={{fontSize:13, color:T.textMuted, marginBottom:12, fontWeight:500}}>
      ...
      {filtered.length===0
        ...
        : <div style={{display:"grid", gap:10}}>
            {filtered.map((r,i) => (
              <RecordCard key={r.id} r={r} cat={cat} delay={i*.025}
                onEdit={()=>onEdit(r)} onDel={()=>onDel(r.id)} onDetail={()=>onDetail(r)} />
            ))}
          </div>
      }
    </div>
...
function RecordCard({ r, cat, delay, onEdit, onDel, onDetail }) {
  ...
  return (
    <div className="fade-up" onClick={onDetail}
      style={{background:T.card, border:`1px solid ${T.border}`, borderLeft:`4px solid ${s.color}`, borderRadius:12, padding:"14px 16px", cursor:"pointer", animationDelay:`${delay}s`, transition:"background .15s, border-color .15s"}}
      onMouseEnter={e=>e.currentTarget.style.background=T.cardHover}
...
const Chip = ({children, color}) => (
  <span style={{background:T.bg, border:`1px solid ${T.borderLight}`, borderRadius:6, padding:"2px 9px", fontSize:12, color:color||T.textSub, fontWeight:500}}>{children}</span>
);
const ABtn = ({onClick, color, children}) => (
  <button onClick={onClick} style={{width:30, height:30, borderRadius:7, border:`1px solid ${color}33`, background:`${color}18`, color, fontSize:13, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center"}}>
    {children}
  </button>
);
...
function EmptyState({ onAdd, onImport, cat }) {
  ...
  return (
    <div style={{textAlign:"center", padding:"60px 20px", background:T.card, borderRadius:14, border:`1px dashed ${T.border}`}}>
      ...
      <div style={{fontSize:13, color:T.textMuted, marginBottom:22}}>Add records manually or import from an Excel file</div>
    ...
    <Overlay onClose={onClose}>
      <div className="slide-up" style={{background:T.sidebar, border:`1px solid ${T.border}`, borderRadius:18, width:"100%", maxWidth:500, maxHeight:"90vh", overflow:"auto"}}>
        ...
        <div style={{padding:"0 22px 22px", display:"flex", gap:10}}>
          ...
          <button onClick={submit}  style={{flex:2, background:cfg.color, border:"none", color:"#000", borderRadius:10, padding:"11px", fontSize:14, fontWeight:700}}>
...
function AlertConfig({ onClose, showToast }) {
  const [cfg, setCfg] = useState(()=>{
    try { return JSON.parse(localStorage.getItem("ct_alertcfg")||"{}"); } catch { return {}; }
  });
